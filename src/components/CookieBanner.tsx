...
export function CookieBanner() {
  ...
  useEffect(() => {
    ...
    if (!consent) {
      // Un mic delay la pornire pentru a lăsa utilizatorul să vadă site-ul mai întâi
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed bottom-6 right-6 md:right-10 z-110 w-[calc(100%-3rem)] sm:w-80 h-auto rounded-3xl bg-white border border-brand-dark/5 [--shadow:rgba(60,64,67,0.3)_0_1px_2px_0,rgba(60,64,67,0.15)_0_2px_6px_2px] [box-shadow:var(--shadow)] font-sans"
          role="alertdialog"
        ...
        >
          <div className="flex flex-col items-center justify-between pt-9 px-6 pb-6 relative">
            ...
            {/* Fursecul Decorativ Custom */}
            <span className="relative mx-auto -mt-16 mb-6 shrink-0 bg-[#fbf9f4] p-3 rounded-full border border-brand-dark/5 shadow-sm">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" height={46} width={65} className="drop-shadow-sm">
                <path stroke="#634647" strokeWidth="1" fill="#EAB789" d="M49.157 15.69L44.58.655l-12.422 1.96L21.044.654l-8.499 2.615-6.538 5.23-4.576 9.153v11.114l4.576 8.5 7.846 5.23 10.46 1.96 7.845-2.614 9.153 2.615 11.768-2.615 7.846-7.846 1.96-5.884.655-7.191-7.846-1.308-6.537-3.922z" />
            ...
            {/* Titlu Secțiune */}
            <h5 id="cookie-title" className="text-sm font-semibold mb-2 text-left mr-auto text-zinc-700 font-sans">
              Confidențialitatea ta este importantă
            </h5>

            {/* Descriere Text */}
            <p id="cookie-desc" className="w-full mb-4 text-xs text-justify text-zinc-500 leading-relaxed font-sans">
              Folosim module cookie pentru a măsura și îmbunătăți performanța site-ului, oferindu-ți o experiență personalizată și delicioasă. 
              <br />
              Pentru mai multe detalii, consultă pagina de{" "}
              <Link 
                to="/politica-cookies" 
                onClick={handleDecline} 
                className="text-xs cursor-pointer font-semibold transition-colors hover:text-[#e91e63] text-[#634647] underline underline-offset-2"
              >
                Politica de Cookie-uri
              </Link>
            </p>

            {/* Buton Refuză (More Options / Decline) */}
            <button 
              onClick={handleDecline}
              className="mb-2 text-xs mr-auto text-zinc-500 cursor-pointer font-semibold transition-colors hover:text-[#634647] hover:underline underline-offset-2 font-sans"
            >
              Refuză
            </button>

            {/* Butonul de Acceptare Finală */}
            <button 
              onClick={handleAccept}
              className="absolute font-semibold right-6 bottom-6 cursor-pointer py-2 px-8 w-max break-keep text-xs rounded-xl transition-all text-[#634647] hover:text-white bg-[#ddad81] hover:bg-[#634647] font-sans" 
              type="button"
            >
              Acceptă
            </button>
